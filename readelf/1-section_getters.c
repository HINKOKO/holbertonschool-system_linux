...
void *get_section_header_start(unsigned char *bytes, int class, int endian)
{
	...
	if (endian == ELFDATA2MSB)
		swapper((unsigned char *)hstart, class == ELFCLASS32 ? 4 : 8);
	return (hstart);
}
...
{
	uint16_t shnum = class == ELFCLASS32 ? ((Elf32_Ehdr *)bytes)->e_shnum
...
{
	uint16_t shead_size = class == ELFCLASS32 ? ((Elf32_Ehdr *)bytes)->e_shentsize
